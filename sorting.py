...
def _measure_memory(sort_fn, data):
    tracemalloc.start()
    sort_fn(data[:])
...
def compare_sorts(data):
    ...
    res_q  = qs.sort(data)
    time_q = (time.perf_counter() - t0) * 1_000_000
    t0     = time.perf_counter()
    res_m  = ms.sort(data)
...
def full_compare(data):
    ...
    res_m  = ms.sort(data)
    time_m = (time.perf_counter() - t0) * 1_000_000
    mem_q  = _measure_memory(qs.sort, data)
    mem_m  = _measure_memory(ms.sort, data)
    return res_q, res_m, time_q, time_m, mem_q, mem_m